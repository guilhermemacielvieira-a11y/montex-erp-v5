...
//
// SEGURANÇA (gate x-push-key): o endpoint NÃO é mais público. O
// segredo vive no Vault do Supabase ('push_api_key'); o pg_cron envia
// o header x-push-key lido do Vault e a função compara com o valor
// obtido via RPC get_push_key (SECURITY DEFINER, só service_role).
// Sem header válido → 401.
//
...
const CORS = { 'Access-Control-Allow-Origin': '*', 'Access-Control-Allow-Methods': 'POST, OPTIONS', 'Access-Control-Allow-Headers': 'authorization, content-type, x-push-key' };
...
// ---- GATE x-push-key (segredo no Vault, lido via RPC service-only) ----
const sentKey = req.headers.get('x-push-key') || '';
const { data: expectedKey, error: keyErr } = await admin.rpc('get_push_key');
if (keyErr) return json({ error: 'config: ' + keyErr.message }, 500);
if (!expectedKey || sentKey !== String(expectedKey)) return json({ error: 'unauthorized' }, 401);
