...
export default function ProducaoMobile() {
  ...
  const obrasAtivas = useMemo(
    () => obras.filter(o => o.status !== 'concluida').sort((a, b) => (a.nome || '').localeCompare(b.nome || '')),
    [obras]
  );

  const pecasFiltradas = useMemo(() => {
    let lst = pecas;
    if (obraFiltro !== 'todas') lst = lst.filter(p => p.obra_id === obraFiltro || p.obraId === obraFiltro);
    if (q.trim()) {
      ...
      <div className="sticky top-0 z-10 bg-slate-950/95 backdrop-blur-md border-b border-slate-800 px-4 py-3 space-y-2">
        ...
        <div className="flex gap-2 overflow-x-auto -mx-1 px-1 pb-1 no-scrollbar">
          <button
            ...
            className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold transition ${obraFiltro === 'todas' ? 'bg-amber-500 text-slate-950' : 'bg-slate-900 border border-slate-800 text-slate-300'}`}
          >Todas</button>
          {obrasAtivas.map(o => (
            <button
              key={o.id}
              onClick={() => setObraFiltro(o.id)}
              className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold transition truncate max-w-[180px] ${obraFiltro === o.id ? 'bg-amber-500 text-slate-950' : 'bg-slate-900 border border-slate-800 text-slate-300'}`}
            >{o.codigo || o.nome}</button>
          ))}
        </div>
      </div>
